...
import { BasePlanViewer, BasePlanSection } from "./BasePlanViewer";
...
/**
 * ImplementationPlanViewer - Displays detailed implementation plans using BasePlanViewer
 */
export function ImplementationPlanViewer({ 
  ...
  const sections: BasePlanSection[] = [];

  // Approach
  sections.push({
    title: "Approach",
    icon: TrendingUp,
    content: <p className="text-sm text-muted-foreground">{plan.approach}</p>
  });

  // Similar functionality
  if (codebaseAnalysis.similarFunctionality.length > 0) {
    sections.push({
      title: "Similar Functionality Found",
      icon: GitBranch,
      content: (
  ...
  // File breakdown
  if (plan.fileBreakdown.length > 0) {
    sections.push({
  ...
  if (plan.integrationStrategy.existingFiles.length > 0 || 
  ...
  if (hasConflicts) {
    alerts.push({
      ...
      description: `${codebaseAnalysis.duplicates.conflicts.length} conflict(s) found. Review carefully before proceeding.`
    });
  }
  if (hasDuplicates && !hasConflicts) {
    alerts.push({
      type: "default" as const,
      icon: Info,
      title: "Duplicates Found",
      description: `${codebaseAnalysis.duplicates.duplicates.length} duplicate(s) will be consolidated.`
    });
  }
